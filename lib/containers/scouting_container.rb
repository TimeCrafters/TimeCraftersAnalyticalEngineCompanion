class ScoutingContainer < Container
  ...
  def populate_fields(array, period)
    ...
    if period == :autonomous
      text "Autonomous", 0, 10, Text::SIZE_HEADING, AUTONOMOUS_HEADER_COLOR, :center
      scouting_data = AppSync.team_scouting_data(:autonomous)
      set_layout_y(50, Text::SIZE)

    elsif period == :teleop
      text "TeleOp", 0, relative_y(@elements.last.y)+@elements.last.height+32, Text::SIZE_HEADING, TELEOP_HEADER_COLOR, :center
